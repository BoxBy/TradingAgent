...
## ✨ 주요 기능
  ...
  * **🌐 다중 API 키 관리**: 여러 개의 API 키를 등록하여 사용량 제한(Rate Limit)에 도달했을 때 자동으로 다른 키로 전환하여 안정적인 서비스 운영을 보장합니다.

## 🏗️ 시스템 아키텍처

TradingAgents는 다음과 같은 단계로 작동하는 정교한 시스템입니다.

1.  **데이터 수집 (Data Ingestion)**: `DataIngestion` 모듈이 Finnhub, Naver 등에서 시장 뉴스 및 종목 데이터를 수집하여 SQLite 데이터베이스에 저장합니다.
...
│   │   └── main.py          # 메인 오케스트레이터
│   ├── services/            # 데이터베이스 및 RAG 관리
│   │   ├── database_manager.py # 뉴스 DB 관리
...
## 📄 주요 파일 설명
  ...
  * **`src/config.py`**: 모든 API 키, 파일 경로, 투자 전략(공격형/안정형) 등 시스템의 주요 설정을 관리합니다.
  * **`src/orchestrators/main.py`**: 각 전문 에이전트들의 분석 결과를 취합하고, 포트폴리오 최적화를 거쳐 최종 투자 결정을 내리는 프로젝트의 "두뇌"와 같은 역할을 합니다.
  * **`src/agents/market.py`**: 시장의 전반적인 상황을 분석합니다. 뉴스를 분석해 관심 종목을 찾아내고, 시장 위험도를 판단해 투자 기준을 동적으로 조절합니다.
  * **`src/agents/stock.py`**: 개별 종목에 대한 심층 분석을 수행합니다. 기술적, 기본적, 정성적, 감성 분석 등 다양한 관점에서 주식을 평가합니다.
  ...
  * **`src/data_providers/kis_wrapper.py`**: 한국투자증권 API를 안정적으로 호출하고, 실시간 환율 정보를 조회하는 등 실제 증권사와의 모든 통신을 담당합니다.
