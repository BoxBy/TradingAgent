...
def run_decision_process(
    ...
    try:
        ...
        reserve_cash = total_assets * min_reserve_ratio

        # 실제 투자 가능한 최대 금액을 계산합니다. (현재 현금 - 예비금)
        if market_type == "US":
            max_investable_cash = (cash_balance - reserve_cash)  / kis_api.get_exchange_rate()
        else:
            max_investable_cash = cash_balance - reserve_cash
        ...
        final_decisions_payload = orchestrator.get_batch_trade_decisions(
            all_stocks_data, balance_info, market_conditions, max_investable_cash * 0.9, market_type
        )
        ...
        for decision in final_decisions:
            ...
            # --- 매수 결정 로직 ---
            if score >= buy_threshold and quantity > 0:
