...
class StockAnalysisTools:
    ...
    def search_additional_news(self, query: str, stock_code: str) -> str:
        ...
        """
        log.info(
            f"Tool 'search_additional_news' called with query: '{query}' for stock: {stock_code}"
        )
        try:
            end_date = datetime.now().strftime("%Y-%m-%d")
            start_date = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d")
            news_list: List[Dict] = self.data_ingestor.get_company_news(
            ...
            filtered_news = [
                ...
                if query.lower()
                in (news.get("headline", "") + news.get("summary", "")).lower()
            ]
            if not filtered_news:
                latest_headlines = "\n".join(
                    [f"- {n['headline']}" for n in news_list[:5]]
                )
                return (
                    f"No news matching '{query}'. Latest headlines:\n{latest_headlines}"
                )
