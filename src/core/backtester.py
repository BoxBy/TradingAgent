...
class LLMStrategyBacktester:
    ...
    def run(self, watchlist: List[str]):
        log.info(
            f"--- Starting LLM Strategy Backtest from {self.start_date.date()} to {self.end_date.date()} ---"
        )
        ...
        for current_date in pd.date_range(
            ...
            log.info(
                f"--- Backtesting for date: {current_date.strftime('%Y-%m-%d')} ---"
            )
            ...
            decisions = orchestrator.get_batch_trade_decisions(
                all_stocks_data, self._get_current_balance()
            )
    ...
    def _gather_data_for_date(
        self, watchlist: List[str], current_date: datetime
    ) -> Dict:
        all_data = {}
    ...
    def _execute_virtual_trades(
        self, decisions: List[Dict], all_stocks_data: Dict, current_date: datetime
    ):
            ...
            if action == "BUY":
                ...
                if self.cash >= cost:
                    ...
                    self.trade_log.append(
                        f"{current_date.date()}: BUY {quantity} {stock} @ {price:.2f}"
                    )
                ...
                self.trade_log.append(
                    f"{current_date.date()}: SELL {quantity} {stock} @ {price:.2f}"
                )
    ...
    def _record_portfolio_value(self, all_stocks_data: Dict, current_date: datetime):
        ...
        self.portfolio_history.append(
            {"Date": current_date, "Value": self.cash + stock_value}
        )
    ...
    def _calculate_performance_metrics(self) -> Dict:
        ...
        sharpe_ratio = (
            (cagr / annualized_volatility) if annualized_volatility != 0 else 0
        )
