...
class TradeMonitor:
    ...
    def _save_state(self):
        ...
        """
        log.info("save state to json file")
        temp_file = self.STATE_FILE + ".tmp"
        try:
            log.info(f"Data to be saved: {self.active_trades}")
    ...
    def register_trade(
        ...
        # Normalize stop loss percent to positive value
        stop_loss_value = abs(stop_loss_value)
        ...
        self.trade_logs_to_add.append(trade_info)
    ...
    def check_and_execute_sells(self):
        ...
        for stock_code, trade_info in list(self.active_trades.items()):
            ...
            def _fmt(price: float) -> str:
                return (f"${price:,.2f}" if market == "US" else f"₩{price:,.0f}")

            if current_price >= trade_info["target_price"]:
                reason = f"Target price reached ({_fmt(current_price)} >= {_fmt(trade_info['target_price'])})"
            elif (
                ...
                if stop_loss_type == "fixed":
                    ...
                    if current_price <= stop_price:
                        reason = f"Fixed stop-loss triggered ({_fmt(current_price)} <= {_fmt(stop_price)})"
                elif stop_loss_type == "trailing":
                    ...
                    stop_price = trade_info.get("stop_loss_price", 0)
                    if current_price <= stop_price:
                        reason = f"Trailing-stop triggered ({_fmt(current_price)} <= {_fmt(stop_price)})"
                    ...
                    if hist_data is not None and not hist_data.empty:
                        ...
                        if current_price <= stop_price:
                            reason = f"ATR stop-loss triggered ({_fmt(current_price)} <= {_fmt(stop_price)})"

            if reason:
                notification.send_notification(f"\nSell condition met for {stock_code}: {reason}")
                quantity_to_sell = current_stock_info["quantity"]
                sell_ok = self.kis_wrapper.place_sell_order(
                    stock_code, quantity_to_sell, price=current_price, market=market
                )
                if not sell_ok:
                    log.warning(f"Sell order failed for {stock_code} ({quantity_to_sell} @ {current_price}) on market {market}. Keeping trade active.")
                    notification.send_notification(
                        f"⚠️ 매도 주문 실패: {ticker_utils.format_for_slack(stock_code)} | {quantity_to_sell}주 @ {'$' if market == 'US' else '₩'}{current_price:,.2f}. 사유: {reason}"
                    )
                    continue
                ...
                # Prepare trade_data for LLM insight generation
                llm_trade_data = {
                    "stock_code": stock_code,
                    ...
                    "sell_reason": reason,
                    "market_type": trade_info.get("market_type", "N/A"),
                ...
                self.rag_manager.generate_and_add_llm_insight(llm_trade_data) # Call the new LLM insight method
                del self.active_trades[stock_code]

        self._save_state()

    def flush_rag_updates(self):
        if self.trade_logs_to_add:
            self.rag_manager.add_trade_logs_to_db(self.trade_logs_to_add)
            self.trade_logs_to_add.clear()
    ...
    def update_trade_parameters(self, stock_code: str, new_params: dict):
        ...
        if new_stop_loss_value is not None:
            is_updated = True
            trade_info["stop_loss_value"] = abs(float(new_stop_loss_value))
            ...
            log.info(
                f"New stop loss for {stock_code} set to: {trade_info['stop_loss_price']:.2f}"
            )
