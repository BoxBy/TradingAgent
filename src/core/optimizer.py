...
class PortfolioOptimizer:
    ...
    def __init__(self, historical_data: Dict[str, pd.DataFrame]):
        ...
        if self.prices_df.empty or len(self.prices_df.columns) < 2:
            raise ValueError("포트폴리오 최적화를 위해서는 최소 2개 이상의 종목 데이터가 필요합니다.")
    ...
    def _calculate_portfolio_performance(self, weights, returns, cov_matrix, risk_free_rate):
        portfolio_return = np.sum(returns.mean() * weights) * 252
        portfolio_std = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights))) * np.sqrt(252)
        sharpe_ratio = (portfolio_return - risk_free_rate) / portfolio_std if portfolio_std != 0 else -1
    ...
    def find_optimal_weights_for_sharpe(self, risk_free_rate=0.02) -> Dict[str, float]:
        ...
        def neg_sharpe_ratio(weights):
            return -self._calculate_portfolio_performance(weights, returns, cov_matrix, risk_free_rate)[2]
        ...
        optimal_weights = {ticker: weight for ticker, weight in zip(self.asset_names, opt_results.x)}
