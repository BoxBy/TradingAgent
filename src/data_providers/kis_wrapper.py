...
class TradingInterface:
    ...
    def __init__(self, mock_trading: bool = config.MOCK_TRADING):
        ...
        try:
            key_info = {
                "appkey": config.KIS_MOCK_APP_KEY if self.mock_trading else config.KIS_APP_KEY,
                "appsecret": config.KIS_MOCK_APP_SECRET if self.mock_trading else config.KIS_APP_SECRET
            }
            acc_no = config.KIS_MOCK_ACCOUNT_NO if self.mock_trading else config.KIS_ACCOUNT_NO
            if not acc_no or '-' not in acc_no:
                raise ValueError(f"계좌번호 형식이 올바르지 않습니다. '{acc_no}' (하이픈 포함 필요)")
            account_info = {
            ...
            if self.mock_trading:
                ...
                self.broker = Api(key_info=key_info, domain_info=domain_info, account_info=account_info)
    ...
    def get_exchange_rate(self, from_currency: str = "USD", to_currency: str = "KRW") -> float:
        ...
        try:
            ...
            log.info(f"Current {from_currency}/{to_currency} exchange rate from forex-python: {rate:.2f}")
            return rate
        except Exception as e:
            log.error(f"Error fetching exchange rate from forex-python: {e}. Using default rate.")
    ...
    def get_balance(self, market_type: str = 'ALL') -> dict:
                    ...
                    if kr_stocks:
                        for stock in kr_stocks:
                            if int(stock.get('hldg_qty', 0)) > 0:
                                portfolio_kr.append({
                                    "stock_code": stock['pdno'], "quantity": int(stock['hldg_qty']),
                                    "average_price": float(stock['pchs_avg_pric']), "current_price": float(stock['prpr']),
                                    "pnl_percent": float(stock['evlu_pfls_rt']), "market_type": "KR"
                                })
                    
                    kr_summary = kr_balance_response.get('output2', [])
                    if kr_summary:
                        cash_balance_krw = int(kr_summary[0]['dnca_tot_amt'])
                        total_assets_krw = int(kr_summary[0]['nass_amt'])
                        prev_day_assets_krw = int(kr_summary[0]['bfdy_tot_asst_evlu_amt'])
                    
                    log.info("국내 계좌 통합 조회 성공.")
                    break
                except Exception as e:
                    log.warning(f"국내 계좌 통합 조회 실패 (시도 {attempt + 1}/{max_retries}): {e}")
        ...
        if market_type.upper() in ['ALL', 'US']:
                    ...
                    if os_stock_list_res and os_stock_list_res.body:
                        ...
                        if os_stocks:
                            for stock in os_stocks:
                                ...
                                if quantity > 0:
                                    portfolio_us.append({
                                        "stock_code": stock.get('ovrs_pdno'), "quantity": quantity,
                                        "average_price": float(stock.get('pchs_avg_pric', 0)), "current_price": float(stock.get('now_pric2', 0)),
                                        "pnl_percent": float(stock.get('evlu_pfls_rt', 0)), "market_type": "US"
                                    })
            ...
            for attempt in range(max_retries):
                try:
                    params = {
                        "CANO": self.broker.account.account_code, "ACNT_PRDT_CD": self.broker.account.product_code,
                        "WCRC_FRCR_DVSN_CD": "02", "NATN_CD": "840", "TR_MKET_CD": "00", "INQR_DVSN_CD": "00"
                    }
                    req = APIRequestParameter(
                        '/uapi/overseas-stock/v1/trading/inquire-present-balance',
                        'VTRP6504R' if self.mock_trading else 'CTRP6504R', params
                    )
                    res = self.broker._send_get_request(req)
                    os_summary = res.body.get('output3', {})
                    if os_summary:
                        overseas_total_krw = float(os_summary.get('tot_asst_amt', 0))
                        # market_type이 'US'일 경우, 국내 자산이 없으므로 해외 자산을 총자산으로 설정
                        if market_type.upper() == 'US':
                            total_assets_krw = overseas_total_krw
                            # 현금 잔고는 국내 계좌에만 있으므로 별도 조회 필요
                            cash_balance_krw = self._get_kr_buyable_cash()
                        else: # 'ALL'일 경우
                            total_assets_krw += overseas_total_krw
    ...
    @kis_api_rate_limiter
    def fetch_historical_data(self, stock_code: str, market: str = 'US', timeframe: str = 'D', period: int = 100) -> pd.DataFrame:
        if not self.broker: return None
        try:
            if market.upper() == 'US':
                log.warning(f"The provided pykis version does not support overseas historical data. Skipping for {stock_code}.")
                return None
    ...
    def get_portfolio(self) -> list:
        ...
        return balance.get("portfolio", [])

    @kis_api_rate_limiter
    def place_buy_order(self, stock_code: str, quantity: int, price: float = 0, market: str = 'US'):
        if not self.broker: return False # 변경: None -> False
        try:
    ...
    @kis_api_rate_limiter
    def place_sell_order(self, stock_code: str, quantity: int, price: float = 0, market: str = 'US'):
        if not self.broker: return False # 변경: None -> False
        try:
            ...
            if market.upper() == 'US':
                if price <= 0:
                    log.error("Overseas stock orders must be limit orders (price > 0).")
                    return False # 변경: None -> False
                response = self.broker.sell_os_stock("NASD", stock_code, quantity, price)
            else:
