...
from ..utils import logger, notification # ✨ notification 추가

log = logger.get_logger(__name__)

class EmergencyManagerSystem2:
    def __init__(self, kis_wrapper: TradingInterface, trade_monitor: TradeMonitor, llm: BaseChatModel, trade_logger: logger.TradeLogger):
    ...
    def handle_emergency_event(self, event_description: str):
        ...
        affected_stocks = self._identify_affected_stocks(event_description, portfolio_codes)
        ...
        for stock_code in affected_stocks:
            if stock_code not in portfolio_codes: continue
            ...
            stock_info = next((s for s in portfolio if s['stock_code'] == stock_code), None)
            if not stock_info: continue

            if impact == "negative":
                log.warning(f"Impact on {stock_code} is NEGATIVE. Liquidating position based on LLM analysis.")
                quantity = stock_info['quantity']
                current_price = stock_info['current_price']
                
                # ✨ 긴급 선별 매도 알림 추가 ✨
                msg = (f"🚨 [긴급 선별 매도] 종목: {stock_code}\n"
                       f"사유: {event_description}\n"
                       f"LLM 분석: {reasoning}")
                notification.send_notification(msg)

                self.kis_wrapper.place_sell_order(stock_code, quantity, price=current_price, market='US')
                self.logger.log_trade(stock_code, "SELL", quantity, current_price, f"Emergency LLM Sell: {event_description}")
                ...
                if new_gain and new_deadline and stock_code in self.trade_monitor.active_trades:
                    self.trade_monitor.update_trade_targets(stock_code, new_gain, new_deadline)
                else:
                    log.warning(f"Could not readjust targets for {stock_code} due to invalid LLM response or not in active trades.")
