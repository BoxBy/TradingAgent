...
class EmergencyManagerSystem2:
    ...
    def _identify_affected_stocks(self, event: str, portfolio: list) -> list:
        ...
        class AffectedStocksResult(BaseModel):
            __root__: list[str]

        try:
            payload = {"event": event, "portfolio": ", ".join(portfolio)}

            if hasattr(self.llm, "with_structured_output"):
                structured_llm = self.llm.with_structured_output(AffectedStocksResult)
                chain = prompt | structured_llm
                result_obj: AffectedStocksResult = chain.invoke(payload)
                if not result_obj:
                    return []
                return result_obj.__root__
    ...
    def _analyze_impact(self, event: str, stock_code: str) -> dict:
        ...
        class ImpactResult(BaseModel):
            ...
            new_sell_deadline_date: str | None = None

        try:
            payload = {"event": event, "stock_code": stock_code}

            if hasattr(self.llm, "with_structured_output"):
                structured_llm = self.llm.with_structured_output(ImpactResult)
                chain = prompt | structured_llm
                result_obj: ImpactResult = chain.invoke(payload)
                if not result_obj:
