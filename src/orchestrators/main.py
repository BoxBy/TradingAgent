...
from ..agents.stock import (TechnicalAnalysisAgent, SentimentAnalysisAgent, 
                           FundamentalAnalysisAgent, QualitativeAnalysisAgent, ChartPatternAgent)
...
class TradingOrchestrator:
    ...
    def get_batch_trade_decisions(self, all_stocks_data: Dict[str, Dict], balance_info: Dict) -> List[Dict]:
        """
        ...
        final_decisions = self._make_final_batch_decision(comprehensive_analyses, balance_info)
        
        # 3. 매수 결정이 여러 개일 경우, 포트폴리오 최적화를 통해 투자 비중을 결정합니다.
        buy_decisions = [d for d in final_decisions if d.get("decision") == "BUY" and d.get("quantity", 0) > 0]
        if len(buy_decisions) > 1:
            log.info("Optimizing portfolio weights for BUY decisions...")
            buy_stocks_data = {d["stock_code"]: all_stocks_data[d["stock_code"]]["ohlcv"] for d in buy_decisions}
            try:
                ...
                total_investment_amount = balance_info.get("cash_balance", 0) * 0.8 # 현금의 80% 투자
                for decision in final_decisions:
                    ...
                    if stock_code in optimal_weights:
                        ...
                        if price_data is not None and not price_data.empty:
                            ...
                            investment_per_stock = total_investment_amount * optimal_weights[stock_code]
                            decision["quantity"] = int(investment_per_stock / stock_price) if stock_price > 0 else 0
            except Exception as e:
                log.error(f"Portfolio optimization failed: {e}. Proceeding without optimization.")

        # 4. 분석 결과와 최종 결정을 RAG DB에 저장합니다.
        for decision in final_decisions:
            stock_code = decision.get("stock_code")
            if stock_code in comprehensive_analyses:
                self.rag_manager.add_analysis_to_db(stock_code, json.dumps(comprehensive_analyses[stock_code], default=str))
    ...
    def _make_final_batch_decision(self, analyses: Dict, balance: Dict) -> List[Dict]:
        ...
        try:
            ...
            log.error(f"Final batch decision agent failed. Raw response: '{output_str}'. Error: {e}", exc_info=True)
            raise e