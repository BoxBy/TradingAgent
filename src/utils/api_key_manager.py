...
def _get_embedding_model():
    """Lazy-load and cache tokenizer/model for embeddings."""
    tokenizer = AutoTokenizer.from_pretrained(EMBEDDING_MODEL_NAME)
    model = AutoModel.from_pretrained(EMBEDDING_MODEL_NAME)
    model.to(EMBEDDING_DEVICE)
    model.eval()
...
def _encode_texts(texts: List[str]) -> List[List[float]]:
    ...
    with torch.no_grad():
        ...
        cls_embeddings = token_embeddings[:, 0, :]   # use CLS token
...
class LLMProvider:
    ...
    def _increment_usage(self, call_type: str, cost: int = 1):
        ...
        - LLM 호출은 key별로 usage[key_alias]["llm"]에 누적
        - Embedding 호출은 키와 무관하게 전역 합산 usage["_embedding_total"]에만 누적
        """

        if call_type == "embedding":
            total = self.usage.get("_embedding_total", 0)
            self.usage["_embedding_total"] = total + cost
            log.info(f"Embedding usage (global) -> {self.usage['_embedding_total']}")
        else:
            active_key = self.api_keys[self.current_key_index]
            key_alias = self._get_key_alias(active_key)
            if key_alias not in self.usage:
                self.usage[key_alias] = {"llm": 0}
    ...
    def _execute_with_retry(self, call_type: str, func: Callable[..., Any], *args, cost: int = 1, **kwargs) -> Any:
        """
        ...
        for i in range(self.num_keys):
            try:
                ...
                self._increment_usage(call_type, cost=cost)
                return result
    ...
    def invoke_structured(self, input: Any, schema: Type[BaseModel], config: Optional[RunnableConfig] = None) -> BaseModel:
        """Pydantic 스키마를 사용한 guided decoding 헬퍼.

        - ChatGoogleGenerativeAI.with_structured_output(schema)를 내부에서 사용
        - 기존키 순환/사용량 카운팅 로직(_execute_with_retry)을 그대로 재사용
        """

        def api_call():
            llm = self.get_llm()
            structured_llm = llm.with_structured_output(schema)
            return structured_llm.invoke(input, config=config)
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        def api_call():
            # 단일 쿼리를 리스트로 감싸 공통 인코딩 경로 사용
            vecs = _encode_texts([text])
            return vecs[0] if vecs else []

        # 임베딩은 로컬에서 계산되지만, 공통 로깅/상태 관리를 위해 재시도 래퍼를 그대로 사용합니다.
        # 단일 쿼리이므로 cost=1로 카운트합니다.
        return self._execute_with_retry("embedding", api_call, cost=1)
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        # 배치의 문서 수만큼 사용량을 증가시킵니다.
        batch_size = len(texts) if isinstance(texts, list) else 1
        return self._execute_with_retry("embedding", api_call, cost=batch_size)
...
class RunnableLLMProvider(Runnable):
    ...
    def with_structured_output(self, schema: Type[BaseModel]) -> "RunnableStructuredLLMProvider":
        """Pydantic 스키마 기반 guided decoding을 위한 Runnable 래퍼를 반환합니다."""
        return RunnableStructuredLLMProvider(provider=self.provider, schema=schema)


class RunnableStructuredLLMProvider(Runnable):
    """LLMProvider.invoke_structured를 사용하는 LangChain Runnable 래퍼.

    - ChatPromptTemplate | RunnableStructuredLLMProvider 형태의 체인에서 사용
    - invoke(input)을 호출하면 Pydantic BaseModel 인스턴스를 반환
    """

    provider: LLMProvider
    schema: Type[BaseModel]

    def __init__(self, provider: LLMProvider, schema: Type[BaseModel], **kwargs: Any):
        super().__init__(**kwargs)
        self.provider = provider
        self.schema = schema

    def invoke(self, input: Any, config: Optional[RunnableConfig] = None) -> BaseModel:
        return self.provider.invoke_structured(input, schema=self.schema, config=config)
