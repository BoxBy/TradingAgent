        ...
        def wrapper(*args, **kwargs):
            ...
            while retries < max_retries:
                ...
                # 여기서는 모든 Exception에 대해 재시도하도록 단순화합니다.
                    ...
                    if isinstance(
                        e, (google_exceptions.ResourceExhausted, OpenaiRateLimitError)
                    ):
                        raise e
                    ...
                    if retries >= max_retries:
                        log.error(
                            f"API call failed after {max_retries} retries for '{func.__name__}'. Error: {e}"
                        )
                        raise e

                    sleep_time = backoff + random.uniform(0, 1)
                    log.warning(
                        f"API call failed for '{func.__name__}'. Retrying in {sleep_time:.2f} seconds... (Attempt {retries}/{max_retries})"
                    )
