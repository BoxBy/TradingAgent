...
class TradeLogger:
    ...
    def _initialize_files(self):
        ...
        if not os.path.exists(self.trades_log_path):
            pd.DataFrame(columns=['Timestamp', 'StockCode', 'Action', 'Quantity', 'Price', 'Reasoning']).to_csv(self.trades_log_path, index=False)
        if not os.path.exists(self.pnl_log_path):
            with pd.ExcelWriter(self.pnl_log_path, engine='openpyxl') as writer:
                pd.DataFrame(columns=['Date', 'TotalAssets', 'DailyPNL', 'DailyReturn_pct']).to_excel(writer, index=False, sheet_name="Performance")

    def log_trade(self, stock_code: str, action: str, quantity: int, price: float, reasoning: str):
        """개별 거래 내역을 CSV 파일에 기록합니다."""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        new_log = pd.DataFrame([[timestamp, stock_code, action, quantity, price, reasoning]],
        ...
        get_logger(__name__).info(f"Trade logged: {action} {quantity} of {stock_code} at {price}")
    ...
    def log_daily_pnl(self, kis_wrapper: 'TradingInterface'):
        ...
        try:
            ...
            try:
                ...
                df = pd.DataFrame(columns=['Date', 'TotalAssets', 'DailyPNL', 'DailyReturn_pct'])
            ...
            if not df.empty:
                ...
                daily_return = (daily_pnl / last_day_assets) * 100 if last_day_assets != 0 else 0
            else: # 첫 거래일
                daily_pnl = 0
                daily_return = 0

            new_pnl_log = pd.DataFrame([[today_str, total_assets, daily_pnl, daily_return]],
                                       columns=['Date', 'TotalAssets', 'DailyPNL', 'DailyReturn_pct'])
            
            df = pd.concat([df, new_pnl_log], ignore_index=True)
            
            with pd.ExcelWriter(self.pnl_log_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
