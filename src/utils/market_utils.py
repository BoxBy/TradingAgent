...
def _is_us_dst(dt: datetime) -> bool:
    if dt.month < 3 or dt.month > 11: return False
    if dt.month > 3 and dt.month < 11: return True
    past_second_sunday_in_march = dt.replace(day=1)
    sundays = 0
    while sundays < 2:
        if past_second_sunday_in_march.weekday() == 6: sundays += 1
        if sundays < 2: past_second_sunday_in_march += timedelta(days=1)
    ...
    if dt.month == 3: return dt >= past_second_sunday_in_march
    if dt.month == 11: return dt < past_first_sunday_in_november
    return False
...
def is_us_market_open() -> bool:
    ...
    if et_now.weekday() >= 5 or et_now.date() in us_holidays:
        log.info(f"US Market is closed. Reason: Weekend or Holiday ({et_now.strftime('%Y-%m-%d')})")
        return False
    ...
    if market_open <= et_now.time() <= market_close:
        ...
        log.info(f"US Market is closed. Current time (ET): {et_now.strftime('%H:%M:%S')}")
        return False
...
def is_kr_market_open() -> bool:
    ...
    if kst_now.weekday() >= 5 or kst_now.date() in kr_holidays:
        log.info(f"KR Market is closed. Reason: Weekend or Holiday ({kst_now.strftime('%Y-%m-%d')})")
        return False
    ...
    if market_open <= kst_now.time() <= market_close:
        ...
        log.info(f"KR Market is closed. Current time (KST): {kst_now.strftime('%H:%M:%S')}")
        return False