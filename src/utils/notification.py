...
def send_notification(message: str, channel: str = "#stock_report"):
    ...
    try:
        if not webhook_url or "YOUR/SLACK/WEBHOOK_URL" in webhook_url:
            log.warning(f"Slack Webhook URL is not configured. Skipping notification for message: {message}")
            return
...
def send_log_file_to_slack(channel: str = "#stock_report"):
    ...
    try:
        ...
        data = {
            'channels': channel,
            'initial_comment': f"데일리 트레이딩 로그 파일 ({datetime.now().strftime('%Y-%m-%d')})",
        ...
        if response_json.get("ok"):
            ...
            raise Exception(f"Slack API returned an error: {response_json.get('error')}")
