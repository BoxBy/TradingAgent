...
def plot_and_save_chart(
    ohlcv_df: pd.DataFrame, stock_code: str, analysis_results: dict
):
    """
    ...
    try:
        ...
        if "SMA_20" in plot_df:
            addplots.append(
                mpf.make_addplot(plot_df["SMA_20"], color="blue", width=0.7)
            )
        if "SMA_60" in plot_df:
            addplots.append(
                mpf.make_addplot(plot_df["SMA_60"], color="green", width=0.7)
            )
        if "BBU_20_2.0" in plot_df:
            # fill_between을 사용하여 볼린저 밴드 영역 채우기
            addplots.append(
                mpf.make_addplot(
                    plot_df[["BBU_20_2.0", "BBL_20_2.0"]],
                    color="gray",
                    alpha=0.1,
                    fill_between=True,
                )
